...
#define function that takes half light radius, sersic index, M/L,
#and total mass as input and return sersic function
def Sersic(a,b,c,d,e):
    Ie = (e/d)/(7.2*np.pi*a**2)
    Ir = Ie*np.exp(-7.76*((b/a)**(1.0/c)-1))
    return Ir

#set n=4
sersicfit = Sersic(Re,R,4,ML,Btot)
print len(sersicfit)
#overplot sersic fit to the buldge density profile
plt.semilogy(R,sersicfit,color='red',linestyle='--',label='Sersic Fit')
# Add axis labels
